...
export function GlossaryBlock({
  ...
  const entries = useMemo(() => {
    return Object.entries(dictionary).map(([term, definition]) => ({
      term,
      definition:
        typeof definition === "string" ? definition : definition.definition,
      category:
        typeof definition === "string" ? undefined : definition.category,
  ...
  const filteredEntries = useMemo(() => {
    ...
    return result.sort((a, b) => {
      const comparison = a.term
        .toLowerCase()
        .localeCompare(b.term.toLowerCase());
  ...
  const renderGlossaryItem = (entry: GlossaryEntry) => (
    <motion.div
      ...
      className={cn(
        ...
        layout === "cards" &&
          "bg-card rounded-lg shadow-sm hover:shadow-md p-6",
        "hover:bg-muted/50",
        itemClassName,
      )}
    >
          ...
          <div key={category} className="space-y-4">
            ...
            <div
              className={cn(
                ...
                layout === "cards" &&
                  "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6 divide-y-0",
