...
export function MediaBlock({
  ...
  const multiVideo = isMultiVideo
    ? (videoProps as MultiVideoBlockProps).multiVideo
    : undefined;

  // Get current timestamps based on multiVideo state
  const currentTimestamps = multiVideo
    ? multiVideo.timestamps?.[currentVideoIndex]
    : timestamps;
  ...
  const handlePrev = () => {
    ...
    setCurrentVideoIndex(
      (prev) => (prev - 1 + multiVideo.urls.length) % multiVideo.urls.length,
    );
    setTimeout(() => setIsTransitioning(false), 300);
  };
  ...
  const {
    savedPosition,
    actions: { saveProgress, startProgressSaving, stopProgressSaving },
  ...
  useEffect(() => {
    if (
      mediaType === "Video" &&
      processedUrl &&
      processedUrl.includes("youtube.com/embed/")
    ) {
      if (!window.YT) {
  ...
  const isValidYouTubeURL = (url: string): url is YouTubeURL => {
    return (
      url.match(/^(https:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/.+/i) !== null
    );
  };
  ...
  const renderMedia = () => {
    if (
      mediaType === "Video" &&
      (!processedUrl || !isValidYouTubeURL(processedUrl))
    ) {
          ...
          <div className="mt-4">
            <VideoTimestamps
              timestamps={parseTimestamps(
                Array.isArray(currentTimestamps) &&
                  Array.isArray(currentTimestamps[0])
                  ? (currentTimestamps[currentVideoIndex] as
                      | string
                      | VideoTimestamp[])
